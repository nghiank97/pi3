...
/* ethercat info */
u08 remote_ip[4]={192,168,137,10};
u16 remote_port=0x88A4;
u16 local_port=5002;
u16 len=0;
...
extern void loop(){
	switch(getSn_SR(0)){
		case SOCK_UDP:
			...
			index = 2;
			index += ethercat_udp_load_datagram(&buf[2],LRW, 0x01, 1000, (1<<15)|(1<<14)|(B000<<12), 0, 0, (u08*)"nguyen",6);
			index += ethercat_udp_load_datagram(&buf[index],LWR, 0x02, 2000, (0<<15)|(1<<14)|(B000<<12), 0, 0, (u08*)"khac",4);
			len_of_data = index - 42;
			// EtherCAT frame header
			dummy = (B0001<<12)|(0<<11)+(index-1);
			buf[0] = dummy&0xFF;
			buf[1] = (dummy>>8)&0xFF;
			sendto( 0, buf, index, remote_ip, remote_port);
			break;
		case SOCK_CLOSED:
